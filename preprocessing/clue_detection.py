...
def setup_matcher(triggers, matcher):
    ...
    """
    for elm in triggers:
        pattern = [{'ORTH': tok} for tok in elm.split()]
        matcher.add(elm, mark_clue, pattern)
...
if __name__=="__main__":
    ...
    split = 'test'
    # load data
    data = annotation_reader.load_data_from_tsv(os.path.join(config.get('Files', 'preproc_data'), '{}_{}.tsv'.format(ds, split)))
